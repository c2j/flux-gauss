...
class TestIssue75_DeadReturnBraceBalance:
    ...
    def test_dead_return_not_stubbed_and_balanced(self, tmp_path):
        ...
        assert "Auto-generated stub" not in content, "bare top-level RETURN must not force the procedure into a stub"
...
class TestIssue101_ItestFixtureConstraints:
    ...
    def test_fixture_fk_topology_orders_inserts_and_deletes(self, tmp_path):
        ...
        fixture = out / "src/test/resources/itest-fixtures/gauss_complete_examples_proc_sync_employee_bonus.sql"
        content = fixture.read_text(encoding="utf-8")
        del_positions = {t: content.index(f"DELETE FROM {t};") for t in ("employee_bonus", "departments")}
        ...
        ins_positions = {t: content.index(f"INSERT INTO {t}") for t in ("departments", "employee_bonus")}
...
class TestIssue70_MultiFileStandaloneRoutines:
    def test_same_schema_routines_share_one_service(self, tmp_path):
        out_dir = _run_cli_pipeline(["issue_70_fnc_a.sql", "issue_70_fnc_b.sql"], tmp_path)
    ...
    def test_case_variant_package_names_merge_into_one_service(self, tmp_path):
        ...
        out_dir = _run_cli_pipeline(["issue_70_casefold_upper.sql", "issue_70_casefold_lower.sql"], tmp_path)
    ...
    def test_other_source_file_change_regenerates_merged_service(self, tmp_path):
        ...
        env = {**os.environ, "OGSQL_BIN": fg.OGSQL_BIN}
        first = subprocess.run(command + ["--full"], capture_output=True, text=True, env=env, timeout=120)
...
class TestIssue34_DTO_Entity:
    ...
    def test_out_params_not_use_atomic_reference(self, cached_ast, tmp_path):
        ...
        assert not has_atomic, "Issue #34: OUT params still use AtomicReference. Expected DTO-based return type."

    def test_ddl_generates_entity_class(self, cached_ast, tmp_path):
        sql_file = "issue_34_35_dto_naming.sql"
        out_dir, pkg, cls = _run_pipeline(sql_file, cached_ast, tmp_path)

        entity_dir = Path(out_dir) / "src" / "main" / "java" / fg.BASE_PACKAGE.replace(".", "/") / "entity"
        entity_files = list(entity_dir.glob("*.java")) if entity_dir.exists() else []
        assert len(entity_files) > 0, f"Issue #34: No Entity classes generated from DDL in {entity_dir}"
    ...
    def test_methods_with_many_params_use_dto(self, cached_ast, tmp_path):
        ...
        if match:
            ...
            param_types = re.findall(
                r"\b(String|Long|BigDecimal|Integer|Boolean|int|long|java\.\w+(?:\.\w+)*)\s+\w+", params
            )
            assert len(param_types) <= 3, (
                f"Issue #34: procCreateOrder has {len(param_types)} flat parameters. Expected ≤3 (DTO parameter)."
            )
...
class TestIssue35_MapperNaming:
    ...
    def test_methods_reflect_target_table_or_operation(self, cached_ast, tmp_path):
        ...
        all_methods = re.findall(
            r"(?:public|String|Map|List|Long|int|void|Integer|BigDecimal|Date|Boolean|Object)\s+(\w+)\s*\(", mapper
        )
        ...
        non_semantic = [m for m in all_methods if not any(kw in m.lower() for kw in keywords)]
        assert len(non_semantic) == 0, (
            f"Issue #35: {len(non_semantic)}/{len(all_methods)} methods lack semantic hints: {non_semantic[:5]}"
        )
...
class TestIssue36_TestFileGeneration:
    ...
    def test_service_test_file_exists_on_disk(self, cached_ast, tmp_path):
        ...
        assert test_fp.exists(), f"Issue #36: Test file not generated at {test_fp}"
        assert test_fp.stat().st_size > 100, f"Issue #36: Test file too small ({test_fp.stat().st_size} bytes)"
    ...
    def test_test_file_contains_test_methods(self, cached_ast, tmp_path):
        ...
        assert bool(re.search(r"@Test", test_content)), "Issue #36: Test file has no @Test annotations"
...
class TestIssue38_MAP_PUT:
    ...
    def test_no_map_put_in_service(self, cached_ast, tmp_path):
        ...
        svc = _read_generated(out_dir, _service_path(out_dir, cls))
        assert svc, "Service file not generated"
        assert "__MAP_PUT__" not in svc, "Issue #38: __MAP_PUT__ residual found in Service.java"
    ...
    def test_no_map_put_in_mapper(self, cached_ast, tmp_path):
        ...
        mapper = _read_generated(out_dir, _mapper_path(out_dir, cls))
        if mapper:
            assert "__MAP_PUT__" not in mapper, "Issue #38: __MAP_PUT__ found in Mapper.java"
    ...
    def test_no_map_put_in_xml(self, cached_ast, tmp_path):
        ...
        if xml_content:
            assert "__MAP_PUT__" not in xml_content, "Issue #38: __MAP_PUT__ found in Mapper.xml"
...
class TestIssue39_ThreadSafety:
    ...
    def test_readonly_constants_generated_as_static_final(self, cached_ast, tmp_path):
        ...
        static_final_names = set(re.findall(r"(?:private|public)\s+static\s+final\s+\w+(?:<\w+>)?\s+(\w+)", svc))
        missing = [c for c in expected_constants if c not in static_final_names]
        assert len(missing) == 0, (
            f"Issue #39: Package constants not generated as static final: {missing}. Found: {static_final_names}"
        )
    ...
    def test_mutable_vars_have_thread_safety_mechanism(self, cached_ast, tmp_path):
        ...
        expected_mutable = ["gCurrentUser", "gSessionId", "gDebugMode", "gBatchStatus"]
        # Check that mutable vars exist as fields (not missing entirely)
        all_fields = set(
            re.findall(r"(?:private|public|protected)\s+(?:static\s+)?(?:final\s+)?(\w+(?:<\w+>)?)\s+(\w+)\s*[=;]", svc)
        )
        ...
        has_safety_comment = bool(re.search(r"THREAD[-_\s]?(SAFE|LOCAL)", svc, re.IGNORECASE))
...
class TestIssue40_StringComparison:
    ...
    def test_equality_uses_equals_not_compareto(self, cached_ast, tmp_path):
        ...
        assert len(compareto_eq) == 0, (
            f"Issue #40: Found {len(compareto_eq)} compareTo()==0 patterns. Equality should use .equals()."
        )

        equals_count = len(re.findall(r"\.equals\(", svc))
        assert equals_count > 0, "Issue #40: No .equals() calls found — string equality comparison missing."
...
class TestIssue41_TypeSystem:
    ...
    def test_numeric_params_map_to_bigdecimal_or_long(self, cached_ast, tmp_path):
        ...
        assert p_amount, f"Issue #41: p_amount (NUMERIC) type not BigDecimal/Long in: {sig[:200]}"
    ...
    def test_date_params_map_to_date_types(self, cached_ast, tmp_path):
        ...
        assert p_date, f"Issue #41: p_transfer_date (DATE) type not Date/LocalDate in: {sig[:200]}"
    ...
    def test_boolean_params_map_to_boolean(self, cached_ast, tmp_path):
        ...
        assert p_bool, f"Issue #41: p_is_urgent (BOOLEAN) type not Boolean in: {sig[:200]}"
    ...
    def test_exception_not_mapped_to_string_field(self, cached_ast, tmp_path):
        ...
        exception_as_string = re.findall(
            r"private\s+String\s+(accountNotFound|insufficientFunds|invalidAmount|eAccountClosed)", svc
        )
        assert len(exception_as_string) == 0, f"Issue #41: EXCEPTION variables mapped to String: {exception_as_string}"
    ...
    def test_percent_type_resolves_from_ddl(self, cached_ast, tmp_path):
        ...
        balance_declared_as_string = re.search(r"vFromBalance\s*=\s*(?:new\s+)?String|vFromBalance\s+String", svc)
        assert not balance_declared_as_string, (
            "Issue #41: t_issue41_account.balance%TYPE (NUMERIC) resolved to String. Expected BigDecimal."
        )
    ...
    def test_bigdecimal_comparisons_use_compareto_correctly(self, cached_ast, tmp_path):
        ...
        string_var_compareto = re.findall(r"(vFromBalance|vBalance|pAmount)\.compareTo\([^)]+\)", svc)
        for match_text in string_var_compareto:
            ...
            assert "BigDecimal" in svc or "Long" in svc, (
                f"Issue #41: {match_text} compareTo() on variable that may not be numeric type."
            )
...
class TestIssue44_IfConditionLoss:
    ...
    def test_nested_dynamic_keeps_ifs(self, cached_ast, tmp_path):
        ...
        assert if_count >= 4, f"Issue #44: only {if_count} if keywords — dynamic SQL cleanup removed them"
    ...
    def test_if_else_balance(self, cached_ast, tmp_path):
        ...
        assert elses <= ifs + 3, f"Issue #44: {elses} else vs {ifs} if — dynamic SQL cleanup removed conditions"
...
class TestIssue45_ExceptionHandling:
    ...
    def test_simple_exception_has_catch(self, cached_ast, tmp_path):
        ...
        assert bool(re.search(r"try\s*\{", svc)), "Issue #45: No try block found for EXCEPTION handling."
        assert bool(re.search(r"\bcatch\s*\(", svc)), "Issue #45: No catch block found for EXCEPTION handling."
    ...
    def test_no_data_found_is_null_check_not_catch(self, cached_ast, tmp_path):
        ...
        no_data_catches = re.findall(r"catch\s*\(Exception\s+e\).*?no.data.found", svc, re.IGNORECASE | re.DOTALL)
        assert len(no_data_catches) == 0, (
            "Issue #45: no_data_found treated as catch block. Should use null check for MyBatis null return."
        )
...
class TestIssue46_ChrAsciiSubstr:
    ...
    def test_chr_output_compiles(self, cached_ast, tmp_path):
        ...
        assert len(bogus_chr) == 0, f"Issue #46: Found {len(bogus_chr)} '(char)(String.valueOf...)' patterns."
...
class TestIssue47_LongParseString:
    ...
    def test_step_no_is_string_not_long(self, cached_ast, tmp_path):
        ...
        assert not has_long_step, (
            "Issue #47: v_step_no mistyped as Long. VARCHAR2 should map to String regardless of naming heuristic."
        )
    ...
    def test_no_parselong_on_dotted_string(self, cached_ast, tmp_path):
        ...
        assert len(dotted_parse) == 0, (
            f"Issue #47: Found {len(dotted_parse)} parseLong on dotted strings. e.g.: {dotted_parse[:3]}"
        )
...
class TestIssue48_LongCompareToString:
    ...
    def test_no_compareto_with_string_literal(self, cached_ast, tmp_path):
        ...
        long_cmp_string = re.findall(r'\b\w+\.compareTo\s*\(\s*"', svc)
    ...
    def test_long_compare_to_long_is_fine(self, cached_ast, tmp_path):
        ...
        has_long_compare = bool(
            re.search(r"compareTo\s*\(\s*Long\.valueOf|compareTo\s*\(\s*\d+L?\s*\)|==\s*\d+L?\b", svc)
        )
        assert has_long_compare, "Issue #48: Long-to-Long comparison not using correct Java pattern."
...
class TestIssue49_Varchar2Concat:
    ...
    def test_pro_id_is_string_not_long(self, cached_ast, tmp_path):
        ...
        assert not has_long_pro, "Issue #49: v_pro_id declared as Long but is VARCHAR2. Should be String type."

        # Verify it is actually String
        has_string_pro = bool(re.search(r"\bString\s+vProId\b", svc))
        assert has_string_pro, "Issue #49: v_pro_id not found as String type. VARCHAR2 should map to String."
    ...
    def test_trade_ids_is_string_not_long(self, cached_ast, tmp_path):
        ...
        assert not has_long_trade, "Issue #49: v_trade_ids declared as Long but is VARCHAR2."
    ...
    def test_concat_uses_string_not_long_arithmetic(self, cached_ast, tmp_path):
        ...
        assert len(concat_pattern) > 0, (
            "Issue #49: No string concatenation found for vProId || '...' Expected vProId + \"...\" pattern."
        )
...
class TestIssue54_NestedException:
    ...
    def test_handler_body_has_nested_try_catch(self, cached_ast, tmp_path):
        ...
        assert catch_count >= 2, f"Issue #54: Expected >=2 catch blocks for nested handler, got {catch_count}"
    ...
    def test_three_level_nested_has_all_catches(self, cached_ast, tmp_path):
        ...
        svc = self._gen_svc(cached_ast, tmp_path)
        catch_count = len(re.findall(r"\bcatch\s*\(", svc))
        assert catch_count >= 3, f"Issue #54: Expected >=3 catch blocks for 3-level nesting, got {catch_count}"
...
class TestIssue56_ReturnInHandler:
    ...
    def test_no_data_found_moved_to_try_body(self, cached_ast, tmp_path):
        ...
        assert not_found_pos < first_catch, "no_data_found handler should be in try body, before catch block"
...
class TestIssue61_OuterExceptionBrace:
    ...
    def test_braces_balanced(self, cached_ast, tmp_path):
        svc = self._gen_svc(cached_ast, tmp_path)
        assert svc.count("{") == svc.count("}"), f"Issue #61: brace mismatch {{={svc.count('{')}}} }}={svc.count('}')}"
    ...
    def test_others_if_body_emitted(self, cached_ast, tmp_path):
        ...
        assert '"0".equals(' in svc or "Objects.equals" in svc, "Issue #61: p_o_succeed = '0' should use string equals"
    ...
    def test_others_elsif_body_emitted(self, cached_ast, tmp_path):
        ...
        svc = self._gen_svc(cached_ast, tmp_path)
        assert "WARN:" in svc, (
            "Issue #61: ELSIF body inside EXCEPTION handler was dropped (wrong AST key for elsif body)"
        )
...
class TestIssue60_InstrCaseWhen:
    ...
    def test_no_bare_one_dot_equals(self, cached_ast, tmp_path):
        svc = self._gen_svc(cached_ast, tmp_path)
        assert "1.equals(" not in svc, "Issue #60: bare 1.equals() — Java parses as float literal"
        assert re.search(r"\+\s*1\.equals\(", svc) is None, "Issue #60: indexOf()+1.equals() operator precedence bug"

    def test_instr_result_parenthesized(self, cached_ast, tmp_path):
        svc = self._gen_svc(cached_ast, tmp_path)
        assert "indexOf(" in svc, "INSTR should map to indexOf"
        assert re.search(r"\(String\.valueOf\([^)]+\)\.indexOf\([^)]+\)\s*\+\s*1\)", svc) or re.search(
            r"\([^)]*\.indexOf\([^)]+\)\s*\+\s*1\)", svc
    ...
    def test_case_when_uses_eq_eq_not_equals_on_int(self, cached_ast, tmp_path):
        svc = self._gen_svc(cached_ast, tmp_path)
        assert " == 0" in svc or "==0" in svc or "Objects.equals" in svc, "Issue #60: CASE WHEN 0 comparison missing"
...
class TestIssue62_SubstrHelper:
    ...
    def test_no_inline_math_min_max_substring(self, cached_ast, tmp_path):
        svc = self._gen_svc(cached_ast, tmp_path)
        inline = re.findall(r"\.substring\(\s*Math\.(?:min|max)\(", svc)
        assert len(inline) == 0, f"Issue #62: found {len(inline)} inline Math.min/max substring expansions"
...
class TestIssue64_BigDecimalEmptyInit:
    ...
    def test_no_bigdecimal_eq_empty_string(self, cached_ast, tmp_path):
        svc = self._gen_svc(cached_ast, tmp_path)
        bad = re.findall(r'(?:BigDecimal|Long|Integer|Double|Float)\s+\w+\s*=\s*""\s*;', svc)
        assert len(bad) == 0, f"Issue #64: numeric vars initialized with empty string: {bad}"
...
class TestIssue75_LoopExceptionBrace:
    ...
    def test_no_orphaned_try(self, cached_ast, tmp_path):
        ...
        assert n_try <= n_handler, f"Issue #75: {n_try} try blocks but only {n_handler} catch/finally handlers"
...
class TestIssue103_HandlerCallArgs:
    ...
    def test_handler_cross_pkg_calls_keep_all_args(self, tmp_path):
        ...
        assert 'calleeService.simpleLog(pX, "detail");' in svc, f"handler 2-arg call must keep both args:\n{svc[:1500]}"
...
class TestIssue99_DuplicateParamNames:
    ...
    def test_service_and_test_use_deduped_names(self, cached_ast, tmp_path):
        ...
        svc = _read_generated(out_dir, _service_path(out_dir, cls))
        assert "Object text, Object text2" in svc, f"Service signature must use deduped params, got:\n{svc[:1200]}"
        test_file = _read_generated(
            out_dir,
            str(Path(out_dir) / f"src/test/java/{fg.BASE_PACKAGE.replace('.', '/')}/service/{cls}ServiceTest.java"),
        )
        assert "Object text2 = " in test_file, f"ServiceTest must declare deduped params, got:\n{test_file[:1200]}"
...
class TestIssueFixturesParse:
    ...
    def test_fixture_extracts_procedures(self, sql_file, cached_ast):
        ast = cached_ast[sql_file]
        procs, _, _ = fg.extract_procedures(ast, sql_file)
        assert len(procs) > 0, f"Fixture {sql_file} extracted 0 procedures. Check SQL syntax."
